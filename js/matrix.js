...
// class Matrix
// --------------------------------------------------------------------
var Matrix = function() { this.m = null; }

Matrix.get_translation = function(x, y) { var m = new Matrix(); m.set_translation(x, y); return m; };
Matrix.get_scale = function(x, y) {	var m = new Matrix(); m.set_scale(x, y); return m; };
Matrix.get_rotate = function(r) { var m = new Matrix(); m.set_rotate(r); return m; };		
Matrix.get_rotate_quarter = function(q) { var m = new Matrix(); m.set_rotate_quarter(q); return m; };		
Matrix.prototype.set_translation = function(x, y) { this.m = [ [1, 0, x], [0, 1, y], [0, 0, 1] ]; return this; };
Matrix.prototype.set_scale = function(x, y) { this.m = [ [x, 0, 0], [0, y, 0], [0, 0, 1] ]; return this; };
...
Matrix.prototype.set_rotate_quarter = function(q) {
	var ql = q % 4;
	if (ql == 0) { this.m = [ [1, 0, 0], [0, 1, 0], [0, 0, 1] ]; } 
	else if (ql == 1) { this.m = [ [0, -1, 0], [1, 0, 0], [0, 0, 1]	]; } 
	else if (ql == 2) { this.m = [ [-1, 0, 0], [0, -1, 0], [0, 0, 1] ];	} 
	else { this.m = [ [0, 1, 0], [-1, 0, 0], [0, 0, 1]	]; }
	return this; 
};
...
Matrix.prototype.transform_shape = function(s) {
	for(var i = 0; i < s.n.length; i++) { this.transform_Vector(s.n[i]) };
	return this;
};		
Matrix.prototype.log = function() {
	...
	this.m.forEach(function row(r) {
		logline = '';
		r.forEach(function component(c) { logline = logline + ' ['+ c + '] '; });
		console.log(logline);
	});
};
